...
package: clean all
	cd .. && rm -f *.zip && zip -r radio_telemetry_tools.zip radio_telemetry_tools -x \*.pyc \*.ts \*.ui \*.qrc \*.pro \*~ \*.git\* \*Makefile*
	mv ../radio_telemetry_tools.zip .

upload: package
	plugin_uploader.py radio_telemetry_tools.zip